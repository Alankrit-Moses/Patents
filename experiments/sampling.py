"""Run and summarize the paper's three-sample evaluation procedure."""
...
from .runner import prompt_version_for, run_task
...
def _load_resume_state(path: Path) -> tuple[str | None, set[tuple[str, str, int]]]:
    ...
    if not records:
        return None, set()
    run_id = records[0].get("sampling_run_id") or records[0].get("robustness_run_id")
    # Match released records on task, executor prompt version, and sample index.
    completed = {
...
def run_sampling(args: argparse.Namespace, config: HarnessConfig) -> int:
    if args.samples < 2:
        raise ValueError("Repeated-sampling runs require --samples >= 2")
    if args.temperature <= 0:
        raise ValueError("Repeated-sampling runs require --temperature > 0")
    ...
    if not tasks:
        ...
        return 1
    save_manifest_and_tasks(manifest, build_tasks(manifest, config), config.task_dir)
    ...
    output_path = (
        ...
        / (
            f"outputs-{_slug(sampled_generator.model)}-temp{args.temperature:g}"
    ...
    # Build pending (task, sample_index) units, skipping completed samples.
    ...
    for task in tasks:
        ...
        for sample_index in range(args.samples):
            ...
            if key in completed_sample_ids:
                ...
                continue
            pending.append((task, sample_index))

    def _run_unit(task: dict[str, Any], sample_index: int) -> dict[str, Any]:
        prompt_version = prompt_version_for()
        sample_id = _sample_id(task["task_id"], prompt_version, sample_index)
        failed_attempts: list[dict[str, Any]] = []
        record: dict[str, Any] | None = None
        for attempt_index in range(args.max_attempts):
            attempted = run_task(task, sampled_config, client)
            ...
            if attempt_index + 1 < args.max_attempts:
                print(
                    f"{task['task_id']} sample={sample_index + 1}/{args.samples} "
                    f"attempt={attempt_index + 1}/{args.max_attempts} retry"
                )
        assert record is not None
        record.update(
            {
                "run_type": "repeated_sampling",
                "sampling_run_id": run_id,
                "sampling_prompt_version": prompt_version_for(),
    ...
    def _handle(record: dict[str, Any], task: dict[str, Any], sample_index: int) -> None:
        with write_lock:
            append_jsonl(output_path, record)
            status = "error" if record["errors"] else "ok"
            print(
                f"{task['task_id']} sample={sample_index + 1}/{args.samples} {status}"
            )

    if args.concurrency <= 1:
        for task, sample_index in pending:
            record = _run_unit(task, sample_index)
            _handle(record, task, sample_index)
    else:
        # The client is stateless per call, so sharing it across threads is safe;
        # only the JSONL append needs the lock.
        with ThreadPoolExecutor(max_workers=args.concurrency) as executor:
            futures = {
                executor.submit(_run_unit, task, sample_index): (task, sample_index)
                for task, sample_index in pending
            }
            for future in as_completed(futures):
                task, sample_index = futures[future]
                record = future.result()
                _handle(record, task, sample_index)

    print(f"Sampled outputs: {output_path}")
    return 0
...
def summarize_samples(path: Path, success_threshold: float = 4.0) -> dict[str, Any]:
...
def command_summarize(args: argparse.Namespace) -> int:
    summary = summarize_samples(Path(args.evaluations).resolve(), args.success_threshold)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the paper's repeated-sampling agentic experiments"
    )
    ...
    run_parser.add_argument("--output", help="Repeated-sample output JSONL path")
    run_parser.add_argument(
        "--concurrency",
        type=int,
        default=1,
        help="Number of task/sample units to run in parallel (default 1 = sequential)",
    )
    run_parser.add_argument(
        "--resume",
        action="store_true",
    ...
    summary_parser = subparsers.add_parser(
        "summarize", help="Summarize judged repeated-sample outputs"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    return run_sampling(args, load_config(args.config))
